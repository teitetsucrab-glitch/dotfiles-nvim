...
local c = {
-- 背景 (Background)
	bg        = "#353B35",	-- (粉っぽさを感じるグレーグリーン)
	bg_light  = "#454D45",	-- (カーソル行を目立たせすぎない)
	bg_dark   = "#252925",	-- (暗部)
-- 文字色 (Foreground)
	fg        = "#D0D6D0",	-- (少しグレーがかった白)
	fg_dim    = "#808580",	-- (背景に溶け込むグレー)
	fg_bright = "#EAEFEA",	-- (最も明るい白でも刺激を抑える)
-- アクセント (Accents) - くすみパステル（ダスティカラー）
	red       = "#CC8888",	-- (彩度を落とした赤)
	green     = "#99BB99",	-- (セージグリーン)
	yellow    = "#CCBB88",	-- (枯草色に近い黄)
	blue      = "#88AACC",	-- (デニムのような青)
	purple    = "#BB99BB",	-- (くすんだ藤色)
	cyan      = "#88CCCC",	-- (落ち着いた青緑)
	orange    = "#CC9977",	-- (ベージュに近いオレンジ)
-- その他
	selection = "#505950",	-- (わずかに明るくする程度)
	comment   = "#757C75",	-- (本当に消えそうな文字)
